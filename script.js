...
(function() {
  /* ========== Konfig ========== */
  const WA_PHONE = '25769100065';            // WhatsApp number (no +)
...
Akana kamwe karahaguruka kati: "Nimba amafaranga atamera ku biti, kubera iki amabanki agira amashami?" 🤔😂”`, img: "youth-2.jpg" },
    ...
    if (nextBtn) {
      nextBtn.addEventListener('click', () => { idx++; show(idx); });
    } else {
      console.warn('Next button not found!');
    }

    setInterval(() => { idx++; show(idx); }, 8000);
    show(idx);
  }
 ...
 /* ========== PWA install banner ========== */
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ensurePlaceholders();
    initContactForm();
    // initLpForm();   // ← remove or comment out if not defined
    // renderLpList(); // ← remove or comment out if not defined
    initSpinner();
    updateQr();
    initPwaBanner();
